...
import 'package:ebook/src/screen/pdf_view/custom_flip_book/dummy_dart_html.dart';
import 'package:ebook/src/screen/pdf_view/custom_flip_book/controller/leaf.dart';
import 'package:ebook/src/screen/pdf_view/custom_flip_book/tuple.dart';
...
class FlipBookController extends ChangeNotifier {
  ...
  Tuple2<Leaf?, Leaf?> get currentLeaves {
    ...
    // return secondLeafIndex == leaves.length
    //     ? Tuple2(leaves[secondLeafIndex - 1], leaves[0])
  ...
  void setVsync(TickerProvider vsync) {
    if (leaves.isEmpty) {
      ...
      leaves = List<Leaf>.generate(
          length, (i) => Leaf(index: i, indexOf: length, vsync: vsync, totalPages: totalPages));
    } else {
